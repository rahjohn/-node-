...
exports.admin = function (req, res, callback){
...
exports.view = function (req, res, callback) {
    ...
    async.auto({
            getData: function get_data(callback) {
                Images.all({where: {userId: req.params.id, uploaded:1, imageApproved:1 }}) //Function returns information for all images.  Similar to 'select * from images;'
                    .then(function (getData) {
                        callback(null, getData); //On a successful query the results are returned in the object 'getData'
                    })
                    .catch(function (err) {
            ...
            getData: function get_data(callback) {
                Users.findOne({where: {userName: req.params.userName}})
                    .then(function (result) {
                        Images.all({where: {userId: result.dataValues.userId, uploaded:1, imageApproved:1 }}) //Function returns information for all images.  Similar to 'select * from images;'
                            .then(function (getData) {
                                callback(null, getData); //On a successful query the results are returned in the object 'getData'
                            })
                            .catch(function (err) {
